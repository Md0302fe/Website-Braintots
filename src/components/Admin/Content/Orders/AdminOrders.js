...
const Orders = () => {
  ...
  const mutationsAcceptOrder = useMutationHooks((data) => {
    // call api delete order in system
    const res = OrderServices.acceptOrder(data.orderId, data.access_token);
    return res;
  });
  ...
  return (
    <div className="Wrapper-Admin-User">
      <div className="Main-Content">
        <h5 className="content-title mb-3">Quản lý đơn hàng</h5>
            ...
            <div className="flex ">
              ...
              <div className="w-full flex justify-end items-center ">
                <div className="w-full flex justify-end items-center gap-3">
                  {stateDetails?.status !== "Đang giao" && (
                    <>
                      <Loading isPending={false}>
                        <div
                          onClick={() => HandleCancelOrder(rowSelected)}
                          className="border-2 py-2 px-3 border-black cursor-pointer hover:border-black hover:bg-slate-100"
                        >
                          Hủy đơn hàng
                        </div>
                      </Loading>
                      <Loading isPending={isPendingAccept}>
                        <div
                          ...
                          className="border-2 py-2 px-6 border-green-500 cursor-pointer hover:border-green-600 hover:bg-slate-100"
                        >
                          Xác nhận đơn
                        </div>
                      </Loading>
                  ...
                  {stateDetails?.status === "Đang giao" && (
                    <>
                     Thời gian giao hàng từ : {converDateString(stateDetails?.updatedAt)}
                    </>
                  )}
                </div>
              </div>
